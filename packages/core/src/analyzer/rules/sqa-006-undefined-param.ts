...
// Fires when a ParamRef is found in expressions but the parameter is not in parameters[]
...
export const sqa006UndefinedParam: QueryRule = {
  id: 'SQA-006',
  title: 'Undefined parameter',
  description: 'A parameter reference is used in an expression but the parameter is not declared in the parameters array.',
  severity: 'warn',
    ...
    for (const name of referencedParams) {
      if (!declaredParams.has(name)) {
        diagnostics.push({
          severity: 'warn',
          code: 'SQA-006',
          message: `Parameter '&${name}' is used but not defined`,
        });
      }
    }
